...
def comparison(gene: str, feature: str, dataset:str, specimen: str, entity: str, conn, anentity=None) -> Figure:

    #! Comparison的图,是对于一个entity,在所有疾病类型之间的比较。
    #! 对于第一类数据,只需要一张图,因为一个基因对应表格中的一行。
    #! 对于第二类数据,一个基因可能对应表格中的n行,因此需要选择其中一个来做图,详见原型。
    """
    gene = 'ENSG00000143514' #基因主页所对应的基因 \\
    feature = 'splc' #此处值是范例,实际上需要根据网页决定 \\
    dataset = 'gse120663' #此处值是范例,实际上需要根据网页决定 \\
    specimen = 'pbmc' #此处值是范例,实际上需要根据网页决定 \\
    entity = 'entity'
    ...
    if feature in ['altp','chim','edit','snp','splc']: #第二类数据
        ...
        for disease in diseases['Disease_condition']:
            ...
            for fentity in fentities:
                ...
                diseases_data[fentity][disease.upper()] = list(temp[temp['feature']==fentity].iloc[0,1:].replace({'NA':'nan'}).fillna(0).astype('float')) #TODO 更好的na策略
