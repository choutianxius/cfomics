...
export default function GeneTypeahead ({ dInput }) {
  ...
  const menuRenderer = (results) => (
    <Menu id="main-search-menu">
      {results.map((result, index) => (
        <MenuItem
          key={result.ensembl_gene_id}
          option={result}
          position={index}
          onClick={() => {
            navigate(`/search/gene/${result.ensembl_gene_id}`);
          }}
        >
  ...
  return (
    <AsyncTypeahead
      ...
      useCache={false}
      filterBy={() => true}
      placeholder="HGNC Symbol or Ensembl Gene ID"
