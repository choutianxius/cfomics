...
import { getDiseasesMicrobe, newComparisonCls3 } from '../dataApi/getGeneData';
import MyAlert from '../../../components/MyAlert';
import { beautifyDiseaseCondition } from '../../../utils/beautify';
import NoDataError from '../../../utils/errors/NoDataError';

export default function MicrobeComparisonNew ({
  omics,
  motif,
  dataset,
  specimen,
  ...
  function draw () {
    ...
    setImgLoading(true);
    setImgError(null);
    newComparisonCls3(motif, 'microbe', dataset, specimen, disease1, disease2)
  ...
  useEffect(() => {
    ...
    getDiseasesMicrobe(motif, dataset, specimen)
      .then((data) => {
        if (data.length === 0) {
        ...
        setImgLoading(true);
        newComparisonCls3(motif, 'microbe', dataset, specimen, ddisease1, ddisease2)
          .then((data) => { setImg(data); setImgError(null); })
          .catch((e) => { setImgError(e); })
          .finally(() => { setImgLoading(false); });
      })
  ...
  return (
    <div className="row gx-2">
      ...
      <div className="col-12 col-md-3 pt-2">
        ...
        <div className="mb-2">
          <label className="form-label" style={{ fontSize: '.875rem' }} htmlFor="select_compm_disease_1">
            Disease 1
          </label>
          <select
            className="form-select form-select-sm"
            id="select_compm_disease_1"
            value={disease1}
            onChange={
                (e) => {
        ...
        <div className="mb-2">
          <label className="form-label" style={{ fontSize: '.875rem' }} htmlFor="select_compm_disease_2">
            Disease 2
          </label>
          <select
            className="form-select form-select-sm"
            id="select_compm_disease_2"
            value={disease2}
            onChange={
                (e) => {
