...
import { getStackedBoxPlot } from '../../dataApi/getGeneData';
import LoadingGlower from '../../../../components/LoadingGlower';
import MyAlert from '../../../../components/MyAlert';

export default function StackedBoxPlot ({ gene, omics, feature, dataset, specimen, element }) {
  const [image, setImage] = useState();
  const [isLoading, setIsLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState();
  useEffect(() => {
    setIsLoading(true);
    getStackedBoxPlot(gene, omics, feature, dataset, specimen, element)
      .then((image) => {
        setImage(image);
